...
{
    /// <summary>
    /// Enhanced driver using explicit dependency graph (Phase 3).
    /// Replaces implicit weighted calculations with topology-ordered cascade computation.
    /// Supports both legacy mode (original weights) and new dependency graph mode.
    /// </summary>
    [ExecuteAlways]
    public sealed class BiofieldAuraDriverV2 : MonoBehaviour
    {
        [SerializeField]
        ...
        [Header("Cascade Mode")]
        [SerializeField]
        [Tooltip("Use new dependency graph (Phase 3) or legacy weights")]
        ...
        [SerializeField]
        [Range(0f, 1f)]
        [Tooltip("Environment modifier to scale all dependency strengths")]
        ...
        private void DebugPrintGraph()
        {
            ...
            {
                graphComputer.DebugPrintComputationOrder();
                Debug.Log(graphComputer.VisualizeDependencyGraph());
            }
        }
        ...
        private void ApplyInteractions(bool immediate)
        {
            ...
            {
                ApplyInteractionsUsingDependencyGraph(immediate);
            }
            else
            {
                ApplyInteractionsLegacy(immediate);
            }
        }

        /// <summary>
        /// Apply interactions using explicit dependency graph (Phase 3).
        /// Replaces 80-line implicit calculation with topology-ordered computation.
        /// </summary>
        private void ApplyInteractionsUsingDependencyGraph(bool immediate)
        {
            var contextCollection = controller.GetComponent<BiofieldAuraController>();
            ...
            if (graphComputer == null)
            {
                var contexts = contextCollection.GetComponent<Auras.Core.LayerContextCollection>();
                if (contexts == null)
                {
                    return;
                }
                ...
                graphComputer = new DependencyGraphComputer(
                    contexts,
                    controller.Profile,
                    interactionProfile.LayerDependencies.ToList()
                );
            }

            var computedStates = graphComputer.ComputeLayerStates(environmentModifier);

            foreach (var kvp in computedStates)
            {
                var layerType = (BiofieldLayerType)kvp.Key;
                var state = kvp.Value;
                controller.SetState(layerType, state.intensity, state.integrity);
            }
        ...
        /// <summary>
        /// Apply interactions using legacy weighted calculation.
        /// Original 80-line method from ApplyInteractions().
        /// Kept for backward compatibility.
        /// </summary>
        private void ApplyInteractionsLegacy(bool immediate)
        {
        ...
        private static float GetCircadianSupport(CircadianAlignmentMode mode) => mode switch
        {
        ...
        private static float GetCircadianDisruption(CircadianAlignmentMode mode) => mode switch
        {
        ...
        private static float GetSocialSupport(SocialFieldMode mode) => mode switch
        {
        ...
        private static float GetSocialOverload(SocialFieldMode mode) => mode switch
        {
        ...
        private static float GetInformationClarity(InformationClimateMode mode) => mode switch
        {
        ...
        private static float GetInformationNoise(InformationClimateMode mode) => mode switch
        {
        ...
        private static float GetGroundingBias(BiofieldPracticeMode mode) => mode switch
        {
        ...
        private static float GetReleaseBias(BiofieldPracticeMode mode) => mode switch
        {
        ...
        private static float GetHeartBias(BiofieldPracticeMode mode) => mode switch
        {
        ...
        private static float GetMeditationBias(BiofieldPracticeMode mode) => mode switch
        {
        ...
        private static float GetServiceBias(BiofieldPracticeMode mode) => mode switch
        {
